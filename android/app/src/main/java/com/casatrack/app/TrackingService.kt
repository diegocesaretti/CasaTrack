...
class TrackingService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_ACTIVITY -> {
                ...
                refreshNetwork("activity_transition")
                if (!wifiHold && !wasHolding) requestImmediateFix("activity_transition")
            }
    ...
    private fun refreshNetwork(reason: String) {
        ...
        // Any connected Wi-Fi is treated as a low-mobility state. The only
        // exception is driving, because the Wi-Fi itself may be moving with us.
    ...
    private fun updateNotification() {
        ...
        val detail = when {
            ...
            wifi.connected && mode == ActivityMode.DRIVING -> "$wifiName · driving · GPS activo"
